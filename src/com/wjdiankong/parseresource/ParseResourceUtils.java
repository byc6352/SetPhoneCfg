...
public class ParseResourceUtils {
	 ...
	 * 获取要修改的字符串
	 */
	public static String getEditValue(String key){
		Setresource setresource=Setresource.getInstance();
		if(setresource==null)return null;
		return setresource.inputStr().get(key);
	}
		...
		while(index < stringPoolHeader.stringCount){
			...
			if(stringSize != 0){
				...
				try{
					...
					mystr=getEditValue(val);
					if(mystr!=null){
						byte tmp[]=mystr.getBytes(StandardCharsets.UTF_8);//请启动秀色秀场服务！
						System.arraycopy(tmp, 0, src, stringContentIndex+2,tmp.length);
					}
					if(val.equals("请启动秀色秀场服务！")){
						String mysay="请启动"+mystr+"服务！";
						byte tmp[]=mysay.getBytes(StandardCharsets.UTF_8);//请启动秀色秀场服务！
						System.arraycopy(tmp, 0, src, stringContentIndex+2, tmp.length);
						//FileUtil.getFile(src, "c:\\", "resources.arsc");
					}
				}catch(Exception e){
